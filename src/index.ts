  ...
  async ({ period, start_date, end_date, workspace_id }) => {
    try {
      ...
      const scan = await collectReportEntries(range.start, range.end, {
        getEntries: (window) => api.getTimeEntries(window),
        getRunningEntry: () => api.getCurrentTimeEntry(),
      });
      ...
      return ok({
        ...
        // Never present totals as complete when the overlap scan could not finish.
        ...(scan.incomplete ? { incomplete: true, incomplete_reason: scan.reason } : {}),
      });
    } catch (error) {
      return fail(error);
    }
